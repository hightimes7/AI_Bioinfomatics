...
if (file.exists("RSEM.isoform.counts.matrix.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("RSEM.isoform.counts.matrix.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("RSEM.isoform.counts.matrix", header=T, com='', row.names=1, check.names=F, sep='\t')
    primary_data = as.matrix(primary_data)
}
source("/usr/local/bin/trinityrnaseq-v2.12.0/Analysis/DifferentialExpression/R/heatmap.3.R")
source("/usr/local/bin/trinityrnaseq-v2.12.0/Analysis/DifferentialExpression/R/misc_rnaseq_funcs.R")
source("/usr/local/bin/trinityrnaseq-v2.12.0/Analysis/DifferentialExpression/R/pairs3.R")
source("/usr/local/bin/trinityrnaseq-v2.12.0/Analysis/DifferentialExpression/R/vioplot2.R")
data = primary_data
myheatcol = colorpanel(75, 'purple','black','yellow')
samples_data = read.table("samples.file", header=F, check.names=F, fill=T)
...
write.table(data, file="RSEM.isoform.counts.matrix.minRow10.CPM.log2.centered.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
write.table(sample_cor, file="RSEM.isoform.counts.matrix.minRow10.CPM.log2.centered.sample_cor.dat", quote=F, sep='	')
sample_dist = dist(t(data), method='euclidean')
hc_samples = hclust(sample_dist, method='complete')
pdf("RSEM.isoform.counts.matrix.minRow10.CPM.log2.centered.sample_cor_matrix.pdf")
...
", "RSEM.isoform.counts.matrix.minRow10.CPM.log2.centered") , ColSideColors=sampleAnnotations, RowSideColors=t(sampleAnnotations))
dev.off()
pdf("RSEM.isoform.counts.matrix.minRow10.CPM.log2.centered.prcomp.principal_components.pdf")
prin_comp_data = data
...
write.table(pca$rotation, file="RSEM.isoform.counts.matrix.minRow10.CPM.log2.centered.PCA.prcomp.scores", quote=F, sep="	")
write.table(pca$x, file="RSEM.isoform.counts.matrix.minRow10.CPM.log2.centered.PCA.prcomp.loadings", quote=F, sep="	")
PCA.loadings=pca$x
